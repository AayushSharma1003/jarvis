...
| **Tool use / agency** | 🚧 designed, phase 4 | Web, files, shell, clipboard, screenshots — each shipping *with* its security layer, or not shipping. |
| **Permission engine + taint tracking** | 🚧 designed, phase 4 | [Written up in full](docs/security-model.md); enforcement code is not written yet. |
| **Extensions** | 🚧 designed, phase 4–5 | Manifest-declared permissions, approved on load, running outside the sandbox. |
...
- Every tool carries a risk level: `safe` / `ask` / `dangerous`. Risky calls confirm with
  the exact action shown.
- **Shell always confirms.** No classifier, no denylist, no "safe command" heuristic —
  every one of those has a bypass.
- Filesystem tools are sandboxed to user-chosen roots, resolved through symlinks.
- **Taint tracking:** once untrusted content (a web page, an unknown file) enters the
  conversation, every side-effectful call escalates to confirmation for the rest of the
  session. Prompt injection is assumed, not defended against by hope.
- `web_fetch` carries SSRF guards; the backend binds 127.0.0.1 with a token and an Origin
  check; extensions declare permissions in a manifest and are approved on load.
