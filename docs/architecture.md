...
- **The filesystem sandbox checks resolved paths, not spellings.** `security/sandbox.py` expands, requires absolute, `.resolve()`s (collapsing `..` and following symlinks), then range-checks — so a shortcut *inside* a root pointing out is refused, and Jarvis's own config/data directories are excluded *before* the root test so "inside a root" can never override them. Roots default to Documents/Downloads/Desktop; `roots = []` means no file access, which is a different thing from an absent key.
